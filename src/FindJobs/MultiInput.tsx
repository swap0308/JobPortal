  ...
  return (
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput 
            variant='unstyled' 
            rightSection={<IconSelector className='w-5 h-5'/>} 
            onClick={() => combobox.toggleDropdown()}
            leftSection={
                <div className='text-bright-sun-400 bg-mine-shaft-900 p-1.5 rounded-full m-1'><props.icon/></div>
            }
            >
           <Pill.Group className='px-4'>
            {value.length > 0 ? (
              <>
                {values}
                {value.length > 1 && (
              ...
              <Input.Placeholder className='!text-mine-shaft-300 !text-semibold'>{props.title}</Input.Placeholder>
            )}
      ...
      <Combobox.Dropdown>
        <Combobox.Search
            ...
            placeholder='Search'></Combobox.Search>
        <Combobox.Options>
          {options}
